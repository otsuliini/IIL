...
bool utils::isNumber(const std::string &s)
{
    return !s.empty() && std::find_if(s.begin(), // return true if the string is not empty and if "find_if() == s.end" returns true.
                                      ...
                                      { return !std::isdigit(c); }) == s.end(); // find_if takes the parameters of where to start searching (iterator), where to stop searching (iterator). As well as the condition (lambda function in this case) to check on each element.
                                                                                /*
